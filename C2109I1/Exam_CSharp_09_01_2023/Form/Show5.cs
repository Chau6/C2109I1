    ...
    public partial class Show5 : Form
    {
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("select stuId,stuName,examMark from (select tblStudent.stuId,tblStudent.stuName,examMark, DENSE_RANK() over(order by tblExam.examMark desc) as R from tblExam left join tblStudent on tblStudent.stuId = tblExam.stuId) result where result.R like @stuUsername", con))
                    {
