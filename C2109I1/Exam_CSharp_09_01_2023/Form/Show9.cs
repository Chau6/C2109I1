...
{
    public partial class Show9 : Form
    {
        public Show9()
        {
            InitializeComponent();
        ...
        private void Show9_Load(object sender, EventArgs e)
        {
            ...
            command.CommandText = "select *, COUNT(*) as Total from ( select case when examMark >= 9 then N'Giỏi' when examMark < 9 and examMark >= 7 then N'Khá' when examMark < 7 and examMark >= 5 then N'Trung bình' when examMark < 3 then N'Kém' when examMark is null then N'Chưa thi' else N'Yếu' end as Rating from (select tblExam.examMark from tblStudent left join tblExam on tblStudent.stuId = tblExam.stuId left join tblCourse on tblExam.couId = tblCourse.couId where tblStudent.stuLevel > 2) result) Rating group by Rating order by Rating desc";
